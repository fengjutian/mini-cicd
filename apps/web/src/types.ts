...
export type Deployment = { id: number; projectId: string; status: string; triggerType: string; branch: string; commitSha?: string; commitMessage: string; commitAuthor: string; errorSummary: string; configSource: 'project' | 'repository'; configSnapshot?: string; hasArtifacts: boolean; artifactSourceDeploymentId?: number; queuedAt?: string; startedAt?: string; finishedAt?: string; createdAt: string }
